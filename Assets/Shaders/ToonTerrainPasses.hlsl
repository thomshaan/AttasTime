...
// Toon Terrain shader with ramp lighting
...
#endif
{
    ...
    // Toon Ramp Lighting Implementation
    // Sample the toon ramp texture based on light intensity
    #if SHADOWS_SCREEN
    half4 clipPos = TransformWorldToHClip(GetAbsolutePositionWS(inputData.positionWS));
    half4 shadowCoord = ComputeScreenPos(clipPos);
#else
    half4 shadowCoord = TransformWorldToShadowCoord(GetAbsolutePositionWS(inputData.positionWS));
#endif
    ...
    // Calculate diffuse term using dot product between normal and light direction
    half NdotL = saturate(dot(inputData.normalWS, mainLight.direction));
    // Apply shadow attenuation
    half shadowAttenuation = mainLight.shadowAttenuation;
    half distanceAttenuation = mainLight.distanceAttenuation;
    // Modify NdotL based on shadow
    NdotL *= shadowAttenuation * distanceAttenuation; //This is the diffuse

    // Sample toon ramp texture (you'll need to add this texture to your shader properties)
    half3 toonRamp = SAMPLE_TEXTURE2D(_ToonRampTex, sampler_ToonRampTex, half2(NdotL, 0)).r;
    
    // Combine toon ramp with main light color and albedo
    half3 toonLighting = toonRamp * mainLight.color * albedo;
    
    // Ensure minimum ambient lighting
    half3 ambientLight = inputData.bakedGI * albedo;
    half3 minAmbient = albedo * 0.05h; // Guaranteed minimum base color
    toonLighting += max(ambientLight, minAmbient);

    // Handle additional lights for a more complete lighting model
    ...
    for (uint lightIndex = 0; lightIndex < pixelLightCount; lightIndex++)
    {
        ...
        half NdotL_add = saturate(dot(inputData.normalWS, light.direction));
...
#ifdef TERRAIN_GBUFFER
    half4 color;
    color.rgb = toonLighting;
    color.a = alpha;
    SplatmapFinalColor(color, inputData.fogCoord);

    // Adjust GBuffer data for toon rendering
    inputData.normalWS = inputData.normalWS * alpha;
    smoothness *= alpha;

    BRDFData brdfData;
    InitializeBRDFData(
        toonLighting,  // Use toon lighting as the base color
        metallic, 
        half3(0.0h, 0.0h, 0.0h),  // specular 
        smoothness, 
        alpha, 
        brdfData
    );
...
#else
    // Final color output with toon lighting
    half4 color = half4(toonLighting, 1.0h);
    SplatmapFinalColor(color, inputData.fogCoord);
